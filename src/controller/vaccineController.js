...
const createVaccineSlot = async (req, res) => {
    try {
        ...
        if (!isValidDate(Date)) return res.status(400).send({ status: false, message: "Date should be in YYYY-MM-DD format" })
...
const vaccineAvailable = async (req, res) => {
    ...
    const D = req.body.Date
        const vaccine = await vaccineModel.find({ Date: D }, { timeSlots: 1, Date: 1, _id: 0 }).lean()
console.log(vaccine)
        if (vaccine.length == 0) return res.status(404).send({ status: false, msg: "allTimeSlots available" })

        let timeSlot = {}

        let available10001030 = vaccine.filter(a => { a.timeSlots[0]["10:00AM-10:30AM"] === "Available" ,console.log(a.timeSlots[0]["10:00AM-10:30AM"]) })
